          ...
          <div style="display:flex; gap:15px; align-items:flex-end; flex-wrap:wrap;">
            <div style="flex:1; min-width:250px;">
              ...
              <select id="reportType" onchange="updateReportView()" style="background:var(--panel-2); color:var(--text); border:1px solid var(--border); border-radius:8px; padding:8px; width:100%; height:40px;">
                ...
                <option value="purchases">💰 รายงานสรุปการสั่งซื้อ</option>
              </select>
            </div>
...
window.updateReportView = function () {
  ...
  if (type === "inventory") {
    ...
    if (start && end) {
      filtered = db.issues.filter(i => i.date >= start && i.date <= end);
    }

    const rows = filtered.map(i => `<tr><td>${i.date}</td><td>${i.issueNo || "-"}</td><td>${i.item}</td><td>${i.qty}</td><td>${i.reason}</td></tr>`).join("");
    content = `
          <div class="card">
            <h3>รายงานประวัติเบิกสินค้า ${start && end ? `(ระหว่างวันที่ ${start} ถึง ${end})` : "(ทั้งหมด)"}</h3>
            <table style="width:100%;">
              <thead><tr><th>วันที่</th><th>เลขที่เอกสาร</th><th>สินค้า</th><th>จำนวน</th><th>เหตุผล</th></tr></thead>
              <tbody>${rows || '<tr><td colspan="5" style="text-align:center; padding:20px;">ไม่พบข้อมูลในช่วงวันที่เลือก</td></tr>'}</tbody>
            </table>
          </div>`;
...
async function exportReportsToExcel() {
  ...
  try {
    ...
    if (type === "inventory") {
      ...
      const start = document.getElementById("reportStartDate").value;
      const end = document.getElementById("reportEndDate").value;
      fileName = `issue_history_${start || "all"}_to_${end || "all"}`;
